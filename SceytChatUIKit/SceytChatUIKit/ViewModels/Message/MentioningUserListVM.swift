...
open class MentioningUserListVM: NSObject {
    ...
    public private(set) lazy var memberObserver: DatabaseObserver<MemberDTO, ChatChannelMember> = {
        DatabaseObserver<MemberDTO, ChatChannelMember>(
            request: MemberDTO.fetchRequest()
                ...
                .sort(descriptors: [.init(keyPath: \MemberDTO.user?.firstName, ascending: false)]),
    ...
    public required init(channelId: ChannelId) {
        ...
        provider = ChannelMemberListProvider(channelId: channelId)
        ChannelMemberListProvider.queryLimit = 50
        super.init()
    }
...
public extension MentioningUserListVM {
