...
extension ComposerVC {
    ...
    open class MediaView: View {
        ...
        open func insert(view: AttachmentView, at index: Int = -1) {
            ...
            v.topAnchor.pin(to: stackView.topAnchor, constant: 6)
            v.bottomAnchor.pin(to: stackView.bottomAnchor, constant: -12)
            ...
            if let asset = view.photoAsset {
                if asset.duration > 0 {
                    ...
                    mv?.timeLabel.text = Formatters.videoAssetDuration.format(asset.duration)
                }
                let newSize = CGSize(width: 64 * Self.scale,
...
extension ComposerVC {
    ...
    open class ThumbnailView: View {
        ...
        open override func setup() {
            super.setup()
            closeButton.setImage(Images.closeImage, for: .normal)
...
extension ComposerVC.ThumbnailView {
    ...
    open class MediaView: View {
        ...
        open override func setupLayout() {
            ...
            timeLabel.pin(to: self, anchors: [.leading(4), .bottom(-4), .trailing(0, .lessThanOrEqual)])
...
extension ComposerVC.ThumbnailView {
    ...
    open class FileView: View {
        ...
        open override func setupLayout() {
            ...
            subtitleLabel.topAnchor.pin(to: imageView.centerYAnchor)
        }
    }
}
