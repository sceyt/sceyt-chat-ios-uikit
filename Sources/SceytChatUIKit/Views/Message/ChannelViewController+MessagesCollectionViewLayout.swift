...
public extension ChannelViewController {
    open class MessagesCollectionViewLayout: UICollectionViewFlowLayout {
        ...
        // The data source is presented newest-first, so content-space "top"
        // (y = 0, UI IndexPath(0,0)) is always the NEWEST message. In
        // `.newestAtBottom` the collection view is mirrored (scaleY: -1) so that
        // edge renders at the visual bottom; in `.newestAtTop` it renders upright.
        // Two consequences the layout relies on, and which hold in both orders:
        //   • Short content naturally hugs the newest edge — no anchor shift needed.
        ...
        /// Which visual edge holds the newest message. Pushed here by
        /// `MessagesCollectionView.applyMessageListOrder()`; it only decides
        /// whether attributes carry the mirror.
        ...
        // sticky-separator pinning invalidation, batch-update passes — and the
        ...
        // transform into the attributes makes every application re-assert it, and
        // also covers subclasses that dequeue/configure views through their own
        // overrides. In `.newestAtTop` the transform is simply identity.
        private func presented(_ attributes: UICollectionViewLayoutAttributes?) -> UICollectionViewLayoutAttributes? {
            guard let attributes else { return nil }
            let copy = attributes.copy() as! UICollectionViewLayoutAttributes
            copy.transform = messageListOrder.contentTransform
            return copy
        }

        open override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
            super.layoutAttributesForElements(in: rect)?.compactMap { presented($0) }
        }

        open override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
            presented(super.layoutAttributesForItem(at: indexPath))
        }

        open override func layoutAttributesForSupplementaryView(ofKind elementKind: String, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
            presented(super.layoutAttributesForSupplementaryView(ofKind: elementKind, at: indexPath))
        }

        open override func initialLayoutAttributesForAppearingItem(at itemIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
            let attributes = presented(super.initialLayoutAttributesForAppearingItem(at: itemIndexPath))
            // A message inserted at the newest edge starts one own-height beyond
            // the anchor instead of fading in place — rising from behind the input
            // bar in `.newestAtBottom`, dropping in from above the newest edge in
            // `.newestAtTop`. Only real inserts get this — items merely shifted by
            // the insert animate their frame change normally. The effect is visible
            // only when the controller runs the batch animated (at-newest-edge
            // inserts); suppressed batches jump straight to final.
        ...
        open override func finalLayoutAttributesForDisappearingItem(at itemIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
            presented(super.finalLayoutAttributesForDisappearingItem(at: itemIndexPath))
        }
        ...
        open override func initialLayoutAttributesForAppearingSupplementaryElement(ofKind elementKind: String, at elementIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
            presented(super.initialLayoutAttributesForAppearingSupplementaryElement(ofKind: elementKind, at: elementIndexPath))
        }
        ...
        open override func finalLayoutAttributesForDisappearingSupplementaryElement(ofKind elementKind: String, at elementIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
            presented(super.finalLayoutAttributesForDisappearingSupplementaryElement(ofKind: elementKind, at: elementIndexPath))
        }
