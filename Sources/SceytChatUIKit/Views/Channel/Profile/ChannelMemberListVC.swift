...
//        cell.selectionStyle = .none
    ...
    open func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        NSObject.cancelPreviousPerformRequests(withTarget: memberListViewModel!, selector: #selector(ChannelMemberListVM.search(query:)), object: lastSearchText)
        if let text = searchBar.text, !text.isEmpty {
            lastSearchText = text
            memberListViewModel.perform(#selector(ChannelMemberListVM.search(query:)), with: text, afterDelay: 0.01)
        } else {
            memberListViewModel.cancelSearch()
        }
    }
