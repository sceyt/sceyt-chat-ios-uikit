//
//  GlobalSearchAllFilesViewModel.swift
//  SceytChatUIKit
//
...
/// Loads all file attachments across every channel for the
/// global-search Files tab initial state (no query active).
open class GlobalSearchAllFilesViewModel: NSObject {
    ...
    private let downloadQueue = DispatchQueue(
        label: "com.sceytchat.uikit.globalSearchAllFiles",
        qos: .userInitiated
    )
                ...
                if let message {
                    fileProvider.downloadMessageAttachments(
                        ...
                        if let attachment = message?.attachments?.first(where: { $0.id == attachment.id }) {
                            ...
                            logger.errorIfNotNil(error, "Download global search file attachment")
                        }
                        DispatchQueue.main.async { completion?(layout) }
                    }
                }
...
extension GlobalSearchAllFilesViewModel: ChannelAttachmentListViewModelProviding {
