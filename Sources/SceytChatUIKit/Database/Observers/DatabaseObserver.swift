...
open class DatabaseObserver<DTO: NSManagedObject, Item>: NSObject, NSFetchedResultsControllerDelegate {
    ...
    open func refresh(at indexPath: IndexPath) {
        guard canGetObject(at: indexPath)
        else { return }
        let object = controller.object(at: indexPath)
        controller.managedObjectContext.perform {
            self.controller.managedObjectContext.refresh(object, mergeChanges: true)
        }
    }
    
    open func startObserver(fetchedAllObjects: Bool = true) throws {
        controller.delegate = self
        if fetchedAllObjects {
